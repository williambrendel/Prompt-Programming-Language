...
const checkSectionContent = (content, feedback) => {
  ...
  // Check section titles and associated content.
  const contentLines = [];
  let title;
 ...
 *
 * @param {string} title - The section title (e.g., "REASONING", "FLOW", "TASK")
 ...
 * ];
 * sectionAnalysis("FLOW", contentLines, feedback);
 *
 * @example
 ...
 */
const validateFlowSyntax = (content, contentLines, lineRange, feedback) => {
  ...
  if (flows.length > 1) {
    ...
    // Check if all unnamed flows form a single compounding chain
